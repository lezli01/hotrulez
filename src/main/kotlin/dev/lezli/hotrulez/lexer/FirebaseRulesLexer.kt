...
class FirebaseRulesLexer : LexerBase() {
    ...
    private fun locateToken() {
        ...
        when {
            ...
            current == '*' && nextIs('*') -> finish(FirebaseRulesTokenTypes.STAR_STAR, tokenStart + 2)
    ...
    private fun scanLineComment() {
        ...
        finish(FirebaseRulesTokenTypes.LINE_COMMENT, index)
    }
    ...
    private fun scanBlockComment() {
        ...
        while (index < endOffset - 1) {
            if (buffer[index] == '*' && buffer[index + 1] == '/') {
                finish(FirebaseRulesTokenTypes.BLOCK_COMMENT, index + 2)
                return
            }
            index++
        }
        finish(FirebaseRulesTokenTypes.BLOCK_COMMENT, endOffset)
    }
    ...
    private fun scanString(quote: Char) {
        ...
        while (index < endOffset) {
            ...
            if (escaped) {
                ...
                finish(FirebaseRulesTokenTypes.STRING, index + 1)
                return
            }
            index++
        }
        ...
        finish(FirebaseRulesTokenTypes.STRING, index)
    }
    ...
    private fun scanNumber() {
        ...
        if (buffer[tokenStart] == '0' && (charAt(tokenStart + 1) == 'x' || charAt(tokenStart + 1) == 'X')) {
            ...
            if (hex > tokenStart + 2) {
                finish(FirebaseRulesTokenTypes.NUMBER, hex)
                return
            }
        }
        ...
        finish(FirebaseRulesTokenTypes.NUMBER, index)
    }
    ...
    private fun scanIdentifier() {
        ...
        val type = when {
            text in KEYWORDS -> FirebaseRulesTokenTypes.KEYWORD
            text in CONSTANTS -> FirebaseRulesTokenTypes.CONSTANT
            text in BUILTINS && nextNonWhitespace(index) == '(' -> FirebaseRulesTokenTypes.BUILTIN
            text in FirebaseRulesDiagnostics.ALLOW_OPERATIONS -> FirebaseRulesTokenTypes.OPERATION
            text in BUILTINS -> FirebaseRulesTokenTypes.BUILTIN
    ...
    private fun scanSingleCharacter(current: Char) {
        when (current) {
            ...
            '/' -> finish(FirebaseRulesTokenTypes.PATH_SEPARATOR, tokenStart + 1)
            '{' -> finish(FirebaseRulesTokenTypes.L_BRACE, tokenStart + 1)
            '}' -> finish(FirebaseRulesTokenTypes.R_BRACE, tokenStart + 1)
            '(' -> finish(FirebaseRulesTokenTypes.L_PAREN, tokenStart + 1)
            ')' -> finish(FirebaseRulesTokenTypes.R_PAREN, tokenStart + 1)
            '[' -> finish(FirebaseRulesTokenTypes.L_BRACKET, tokenStart + 1)
            ']' -> finish(FirebaseRulesTokenTypes.R_BRACKET, tokenStart + 1)
            ',' -> finish(FirebaseRulesTokenTypes.COMMA, tokenStart + 1)
            '.' -> finish(FirebaseRulesTokenTypes.DOT, tokenStart + 1)
            ':' -> finish(FirebaseRulesTokenTypes.COLON, tokenStart + 1)
            ';' -> finish(FirebaseRulesTokenTypes.SEMICOLON, tokenStart + 1)
            '$' -> finish(FirebaseRulesTokenTypes.DOLLAR, tokenStart + 1)
            '=' -> {
                val hasEquals = nextIs('=')
                finish(
                    if (hasEquals) FirebaseRulesTokenTypes.OPERATOR else FirebaseRulesTokenTypes.EQUALS,
                    tokenStart + if (hasEquals) 2 else 1,
                )
            }
            '!', '<', '>' -> {
                val hasEquals = nextIs('=')
                finish(FirebaseRulesTokenTypes.OPERATOR, tokenStart + if (hasEquals) 2 else 1)
            }
            '&' -> {
                val hasAmpersand = nextIs('&')
                finish(
                    if (hasAmpersand) FirebaseRulesTokenTypes.OPERATOR else TokenType.BAD_CHARACTER,
            ...
            '|' -> {
                val hasPipe = nextIs('|')
                finish(
                    if (hasPipe) FirebaseRulesTokenTypes.OPERATOR else TokenType.BAD_CHARACTER,
                    tokenStart + if (hasPipe) 2 else 1,
                )
            }
            '+', '-', '*', '%', '?' -> finish(FirebaseRulesTokenTypes.OPERATOR, tokenStart + 1)
