...
import dev.lezli.hotrulez.refactoring.FirestoreRulesNamesValidator
 ...
 */
class FirestoreRulesRenameTest : BasePlatformTestCase() {
    ...
    fun testRenameRecursiveWildcardPreservesRecursiveForm() {
        ...
        val wildcard = PsiTreeUtil.findChildrenOfType(myFixture.file, FirestoreRulesRecursiveWildcard::class.java).first()
    ...
    fun testNamesValidatorRejectsReservedKeywords() {
        val validator = FirestoreRulesNamesValidator()
    ...
    private fun pathWildcardIn(matchPathFragment: String): FirestoreRulesPathWildcard =
        PsiTreeUtil.findChildrenOfType(myFixture.file, FirestoreRulesPathWildcard::class.java).first { wildcard ->
