 ...
 */
class FirestoreRulesEditorBehaviorTest : BasePlatformTestCase() {
    fun testTypingParenInsertsClosingParen() {
        myFixture.configureByText(FirestoreRulesFileType, "allow read: if isOwner<caret>")
        myFixture.type("(")
        assertEquals("allow read: if isOwner()", myFixture.editor.document.text)
    }

    fun testTypingBracketInsertsClosingBracket() {
        myFixture.configureByText(FirestoreRulesFileType, "allow read: if x in roles<caret>")
        myFixture.type("[")
    ...
    fun testTypingDoubleQuoteInsertsPair() {
        myFixture.configureByText(FirestoreRulesFileType, "allow read: if name == <caret>")
        myFixture.type("\"")
    ...
    fun testCommentLineRoundTrips() {
        myFixture.configureByText(FirestoreRulesFileType, "<caret>allow read: if true;")
    ...
    fun testFileTypeExposesIcon() {
        assertNotNull(FirestoreRulesFileType.icon)
    }
}
