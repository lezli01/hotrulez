...
- The parser and typed PSI are generated by Grammar-Kit from
  `src/main/grammar/FirestoreRules.bnf`, and the parsing lexer is generated by
  JFlex from `src/main/grammar/FirestoreRules.flex`. Generated sources land in
  `build/generated/sources/grammarkit` and are wired into the main source set;
  the generate tasks run before `compileKotlin`/`compileJava`.
- The grammar covers the full Firestore Rules surface (version/service/match/
  allow/function/let/return, path patterns with wildcards, recursive wildcards,
  path interpolation and `(default)` segments, and the complete expression
  language with operator precedence, `in`/`is`, ternary, calls, member access,
  subscripts/slices, and list/map literals). It is intentionally permissive so
  the IDE keeps working while editing; constructs not confirmed by official
  Firebase docs are tagged `UNCONFIRMED` in the `.bnf` for future diagnostics.
- The earlier handwritten structural `PsiParser` was replaced by this generated
  grammar. The coarse handwritten lexer is retained solely for syntax
  highlighting (it is context-sensitive in a way the parser lexer need not be).
